  ...
  FROM [YLIKA_KOSTOL].[dbo].[еяца]
	...
	INNER JOIN (SELECT ID AS tmpID29, (datepart(year, [глея_аитгсгс]) * SIN(((datepart(dayofyear, [глея_аитгсгс])) * 360) / 366)) AS [DayOfYearCartesY]
				...
				ON tmp29.tmpID29 = [еяца].ID
															...
															--432646
  WHERE (([дег_пекатгс] <> 1 OR [дег_пекатгс] IS NULL) AND --249528 --Getting only Clients --1=DEH
		 ([айуяыхем] = 0 OR [айуяыхем] IS NULL) AND --239555 --Akyrothen <> 0 = false record, ergo non needed
		 ...
		 ([Label] = 1 OR DATEADD(MONTH,2,[глея_аитгсгс]) < '2016-07-06') AND --213535 --Getting the results with actual label of 0 or 1, not as of yet undecided
		 ...
		 [глеяес_лекетгс] >= 0 AND --182691 --A study cannot have ended before it even began, hence erroneous data (noise) are being cleaned
		 [йостос_еяцатийым_йатасйеугс] IS NOT NULL AND --159326 --Cost is a critical variable, so it must be filled in
		 [йостос_укийым_йатасйеугс] IS NOT NULL AND --155915 --Cost is a critical variable, so it must be filled in
		 [йостос_йатасйеугс] IS NOT NULL AND --155740 --Cost is a critical variable, so it must be filled in
		 [йостос_еяцокабийым_епидосгс] IS NOT NULL AND --153739 --Cost is a critical variable, so it must be filled in

		 [йостос_еяцатийым_йатасйеугс] > 0 AND --153577 --A negative cost points to a dismantlement, which are irrelevant projects
		 [йостос_укийым_йатасйеугс] > 0 AND --150520 --A negative cost points to a dismantlement, which are irrelevant projects
		 [йостос_йатасйеугс] > 0 AND --150486 --A negative cost points to a dismantlement, which are irrelevant projects
		 [йостос_еяцокабийым_епидосгс] > 0 AND --150485 --A negative cost points to a dismantlement, which are irrelevant projects

		 [MONADA] IS NOT NULL AND --150485
		 [йатгцояиа] IS NOT NULL AND --146759
		 [глеяес_лекетгс] IS NOT NULL AND --146759
		 [сумеяцеио_лекетгс] IS NOT NULL AND --146722
		 [ейтасг_еяцоу] IS NOT NULL AND --134618
		 [амацйг_ус] IS NOT NULL AND --134618

		 [глея_аитгсгс] IS NOT NULL AND --134480
		 [глея_йатавыягсгс] IS NOT NULL AND --134479
		 [глея_амаццекиас] IS NOT NULL AND --133122
		 [глея_лекетгс] IS NOT NULL --AND --133098

		 --[SAP_тупос_пекатг] IS NOT NULL AND --28207
		 --[SAP_еидос_аитглатос] IS NOT NULL --AND --21428
		 )